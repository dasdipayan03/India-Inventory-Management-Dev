(function () {
  ...
  function render(m) {
    ...
    const unhealthy=!db.ready || (r.by_status_class?.["5xx"]||0)>0;
  ...
  async function load() { $("problem").hidden=true; $("refresh").disabled=true; $("refresh").textContent="Refreshing…"; try { const response=await fetch("/api/ops/metrics",{credentials:"same-origin",cache:"no-store"}); if(response.status===401||response.status===403){location.replace("/");return;} if(!response.ok) throw new Error(`Report request failed (${response.status})`); const data=await response.json(); if(!data.success) throw new Error(data.error||"Could not load report"); render(data.metrics); } catch(error) { $("problem").textContent=`Could not load the health report: ${error.message}`; $("problem").hidden=false; } finally { $("refresh").disabled=false; $("refresh").textContent="Refresh report"; } }
